...
public class WebServer extends Thread {
	...
	public void run() {
		...
		try {		
			...
			LogUtilities.logString("Webserver: open");
			while(isRunning){
				try {
					...
					Thread proceedRequest = new Thread(){
						...
						public void run() {
							...
							try {
								LogUtilities.logString("Webserver: bind");
								serverConnection.bind(socket, new BasicHttpParams());
								httpService.handleRequest(serverConnection, httpContext);
								serverConnection.shutdown();
								LogUtilities.logString("Webserver: shutdown");
							} catch (IOException e) {
								e.printStackTrace();
							}
					...
					e.printStackTrace();
					LogUtilities.logString("Webserver: error thread");
				} 
			}
			serverSocket.close();
			LogUtilities.logString("Webserver: close");
		} 
		catch (IOException e) {
			e.printStackTrace();
			LogUtilities.logString("Webserver: error socket");
		}
