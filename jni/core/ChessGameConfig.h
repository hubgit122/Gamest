...
namespace CIG
{
    ...
    public:
        ...
        **************************************************************************/
        ...
        Array <Board<CHESSMAN_TYPES>, INI_PLAYER_ARRAY_SIZE, 0> iniBoards;
    ...
    ChessGameConfig::ChessGameConfig(const JSONNode& json): JSON_INI(operationGraph), JSON_INI(iniBoards),JSON_INI(boardRange)
    {
        JSON_GET(thetaOfXTop,float);
        JSON_GET(thetaOfXY,float);

        JSON_GET(lengthOfLattticeX,int);
        JSON_GET(lengthOfLattticeY,int);
        JSON_GET(borderWidth,int);
        JSON_GET(latticePenWidth,int);

        JSON_GET(markCrossByCircle,bool);
        JSON_GET(markCircleRadias,bool);
        JSON_GET(drawLineWhenDrawDot,bool);

        JSON_GET(roundChessman,bool);
        JSON_GET(namedChessman ,bool);

        chessmanRect = json.at("chessmanRect");
        chessmanNames.insert(pair<CHESSMAN_TYPES, string>(CHESS, " "));

        parseArrayWithJsonConstructor(json.at(TO_STRING(LINE_DIRECTION)), LINE_DIRECTION)
