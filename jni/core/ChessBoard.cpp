...
namespace CIG
{
    ...
    Chessboard::Chessboard(const Game &g) : nowTurn(0), nowRound(0), game(&g), players(g.players), pickedChessman(), currentBannedMotions(), chessmanBoard()
    {
    ...
    Chessboard::Chessboard(const JSONNode &json, const Game &g) : nowTurn(json.at("nowTurn").as_int()), nowRound(json.at("nowRound").as_int()), game(&g), players(g.players), pickedChessman(json.at("pickedChessman")), currentBannedMotions(json.at("currentBannedMotions")), chessmanBoard(json.at("chessmanBoard"))
    {
    ...
    //************************************
    Chessman *Chessboard::operator [](PointOrVector p) const           // TO-DO 这里逻辑可以优化一下, 看看怎么检测比较好
    {
    ...
    bool Chessboard::beyondBoardRange(PointOrVector &p)const           // 可以被改写从而提高性能
    {
    ...
    bool Chessboard::onPutIntent(Chessman *c, PointOrVector p, bool refreshEvaluations)
    {
    ...
    void Chessboard::undoPut(Chessman *c, PointOrVector previousP, bool refreshEvaluations /*= false*/)
    {
    ...
    {
        game->nowTurn = (unsigned short)((game->nowTurn + 1) % (MAX_PLAYER_NUM));
        game->nowRound++;
        return true;
    }
    ...
    {
        game->nowTurn = (unsigned short)((game->nowTurn - 1 + MAX_PLAYER_NUM) % (MAX_PLAYER_NUM));
        game->nowRound--;
    }
    ...
    bool Chessboard::onPickIntent(PointOrVector p, bool refreshEvaluations)
    {
        return onPickIntent((*this)[p], refreshEvaluations);
    }

    void Chessboard::undoPick(Chessman *c, PointOrVector p, bool refreshEvaluations)
    {
    ...
    void Chessboard::undoPromotion(Chessman *c, CHESSMAN_TYPES t, bool refreshEvaluations)
    {
        //TODO
    }
    void Chessboard::undoPromotion(PointOrVector p, CHESSMAN_TYPES t, bool refreshEvaluations)
    {
        //TODO
    }

    bool Chessboard::onCaptureIntent(Chessman *c, PointOrVector p, bool refreshEvaluations)
    {
        if(beyondBoardRange(p))
        {
            return false;
    ...
    bool Chessboard::onPromotionIntent(Chessman *c, CHESSMAN_TYPES t, bool refreshEvaluations)
    {
        return false;
    }
    ...
    bool Chessboard::onPromotionIntent(PointOrVector p, CHESSMAN_TYPES t, bool refreshEvaluations)
    {
